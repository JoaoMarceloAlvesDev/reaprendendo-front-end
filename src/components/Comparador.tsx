...
export function Comparador({ valor1, valor2 }: Props) {
    ...
    return (
        <div style={{position: "relative", width: "100%", display: "flex", flexDirection: "row", gap: 20, justifyContent: "center", alignItems: "center", marginTop: 20}}>
            <p className="numeroVotos" style={{left: 10}}>{valor1 > 0 && `${valor1Percent.toFixed(0)}% (${valor1})`}</p>
            ...
            <p className="numeroVotos" style={{right: 10, textAlign: "end"}}>{valor2 > 0 && `(${valor2}) ${valor2Percent.toFixed(0)}%`}</p>
        </div>
    );
}